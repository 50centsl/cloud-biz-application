package com.waytous.cloud.biz.web.rest;
...
import com.waytous.cloud.biz.config.KafkaProperties;
...
class CloudBizGatewayKafkaResourceIT {
    ...
    void setup() {
        ...
        CloudBizGatewayKafkaResource kafkaResource = new CloudBizGatewayKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/cloud-biz-gateway-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/cloud-biz-gateway-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
